...
// Contact form: opens the visitor's email app with your email pre-filled.
// For a static website, this is the safest no-backend option.
...
if (contactForm) {
  contactForm.addEventListener("submit", (event) => {
    ...
    if (!name || !email || !message) {
      if (formMessage) formMessage.textContent = "Please fill all fields before sending.";
      return;
    }
    ...
    if (formMessage) {
      formMessage.textContent = "Opening your email app... Please press Send there.";
    }

    window.location.href = `mailto:avikashr093@gmail.com?subject=${subject}&body=${body}`;
  });
}
